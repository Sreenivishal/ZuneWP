...
private class DelegatingDataSource(
    ...
    private val defaultDataSource by lazy { defaultDataSourceFactory.createDataSource() }
    private val resolvingDataSource by lazy { resolvingDataSourceFactory.createDataSource() }

    override fun addTransferListener(transferListener: androidx.media3.datasource.TransferListener) {
        defaultDataSource.addTransferListener(transferListener)
        resolvingDataSource.addTransferListener(transferListener)
    }

    override fun open(dataSpec: DataSpec): Long {
        val uriString = dataSpec.uri.toString()
        val isLocal = dataSpec.uri.scheme == "content" || 
                      dataSpec.uri.scheme == "file" || 
                      uriString.startsWith("content:") || 
                      uriString.startsWith("file:") || 
                      uriString.startsWith("/")
        
        val dataSource = if (isLocal) {
            Logger.d("DelegatingDataSource", "Using DefaultDataSource for local URI: ${dataSpec.uri}")
            defaultDataSource
        } else {
            resolvingDataSource
        }
...
@UnstableApi
private class DelegatingDataSourceFactory(
    private val context: Context,
    private val defaultDataSourceFactory: DataSource.Factory,
    private val resolvingDataSourceFactory: DataSource.Factory
) : DataSource.Factory {
    override fun createDataSource(): DataSource {
        return DelegatingDataSource(context, defaultDataSourceFactory, resolvingDataSourceFactory)
    }
}
