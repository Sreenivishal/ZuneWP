...
private fun Track.isPodcast(): Boolean =
    videoType.equals("Podcast", ignoreCase = true) ||
        category.equals("Podcast", ignoreCase = true) ||
        resultType.equals("Podcast", ignoreCase = true)

@JvmName("MediaItemtoSongEntity")
@UnstableApi
fun SongEntity.toMediaItem(): MediaItem {
    val isLocal = this.videoId.startsWith("content:") || this.videoId.startsWith("file:") || this.videoId.startsWith("/")
    val isSong = isLocal || (this.thumbnails?.contains("w544") == true && this.thumbnails?.contains("h544") == true)
    return MediaItem
        .Builder()
