...
//        }
    ...
    override fun getSongInfo(videoId: String): Flow<SongInfoEntity?> =
        flow {
            ...
            val isLocal = id.startsWith("content:") || id.startsWith("file:") || id.startsWith("/")
            if (isLocal) {
                emit(null)
                return@flow
            }
    ...
    override suspend fun getLikeStatus(videoId: String): Flow<Boolean> =
        flow {
            val isLocal = videoId.startsWith("content:") || videoId.startsWith("file:") || videoId.startsWith("/")
            if (isLocal) {
                emit(false)
                return@flow
            }
    ...
    override suspend fun addToYouTubeLiked(mediaId: String?): Flow<Int> =
        flow {
            if (mediaId != null) {
                val isLocal = mediaId.startsWith("content:") || mediaId.startsWith("file:") || mediaId.startsWith("/")
                if (isLocal) {
                    emit(0)
                    return@flow
                }
